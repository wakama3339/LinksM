...
export async function GET() { await initializeDb(); const setting = await sql`SELECT theme FROM app_settings WHERE id = 1`; return NextResponse.json({ authenticated: await isLoggedIn(), theme: setting[0].theme }); }
