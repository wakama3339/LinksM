...
export async function PATCH(request: NextRequest) {
  const denied = await requireAuth(); if (denied) return denied; await initializeDb(); const { password, confirmPassword, theme, groupsCollapsed } = await request.json();
  ...
  if (typeof groupsCollapsed === 'boolean') await sql`UPDATE app_settings SET groups_collapsed = ${groupsCollapsed} WHERE id = 1`;
